...
import { MpApprovedDetailItem, mpFetchApprovedDetail } from 'api-definitions/MpAdjustment';
...
export default function MpAdminAdjustmentApprovedDetail() {
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();
  const [data, setData] = useState<MpApprovedDetailItem[]>([]);
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        if (id) {
          const response = await mpFetchApprovedDetail(parseInt(id), {
            page: values.page,
            size: values.size,
        ...
        console.error('Failed to fetch approved detail:', error);
        showError('승인 상세 정보를 불러오는 중 오류가 발생했습니다.');
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const handleBusinessPartnerClick = (id: number) => {
    navigate(`/admin/adjustment/business-partner/${id}`);
  };
    ...
    <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={ko}>
      <Box sx={{ p: 3 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
          <IconButton onClick={() => navigate('/admin/adjustment/approved')} sx={{ mr: 2, p: 1 }}>
