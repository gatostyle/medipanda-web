...
import { MpNotice, MpNoticeSearchRequest, mpFetchNoticeList } from 'api-definitions/MpContent';
...
export default function MpAdminCustomerCenterNoticeList() {
  ...
  const [data, setData] = useState<MpNotice[]>([]);
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      setIsLoading(true);
      try {
        const searchRequest: MpNoticeSearchRequest = {
          page: values.page,
          size: values.size,
        ...
        const response = await mpFetchNoticeList(searchRequest);
  ...
  const handleCreateClick = () => {
    navigate('/admin/customer-center/notice/create');
  };
      ...
      <Box sx={{ p: 3 }}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
          ...
          <Box sx={{ display: 'flex', gap: 1 }}>
            ...
            <Button
              variant="contained"
              onClick={() => openNotImplementedDialog('등록')}
              sx={{
                bgcolor: '#10B981',
                borderRadius: '20px',
                '&:hover': { bgcolor: '#059669' }
