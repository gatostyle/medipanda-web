...
import { MpBlindItem, MpBlindSearchRequest, mpFetchBlindList } from 'api-definitions/MpCommunity';
...
export default function MpAdminCommunityBlindList() {
  const [data, setData] = useState<MpBlindItem[]>([]);
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      setIsLoading(true);
      try {
        const searchRequest: MpBlindSearchRequest = {
          page: values.page,
          size: values.size,
          contentType: values.contentType === '전체' ? undefined : values.contentType,
        ...
        const response = await mpFetchBlindList(searchRequest);
        setData(response.content);
        setTotalElements(response.totalElements);
      } catch (error) {
        console.error('Failed to fetch blind list:', error);
        showError('블라인드 목록을 조회하는 중 오류가 발생했습니다.');
      } finally {
        setIsLoading(false);
      }
    }
    ...
    <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={ko}>
      <Box sx={{ p: 3 }}>
          ...
          <Grid container spacing={2} alignItems="center">
            <Grid item xs={12} sm={2}>
              ...
              <FormControl fullWidth size="small">
                ...
                >
                  <MenuItem value="포스트">포스트</MenuItem>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
          ...
          <Button
            variant="contained"
            onClick={() => openNotImplementedDialog('블라인드 해제')}
              ...
              {isLoading ? (
                ...
                data.map((item, index) => (
                  <TableRow key={item.id} hover>
                    ...
                    <TableCell align="center">{item.contentType}</TableCell>
                    <TableCell
                      align="center"
                    ...
                    <TableCell align="center">{item.reportType}</TableCell>
                    <TableCell align="center">{item.likeCount}</TableCell>
                    <TableCell align="center">{item.blindProcessDate}</TableCell>
                  </TableRow>
                ))
              )}
            </TableBody>
